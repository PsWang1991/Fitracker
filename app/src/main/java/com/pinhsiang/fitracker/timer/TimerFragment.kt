...
class TimerFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        viewModel.displayExerciseMinutes.observe(this, Observer {
            ...
            when {
                viewModel.displayExerciseSeconds.value!! >= 1 ->
                    if (it < 0) viewModel.setDisplayExerciseMinutesTo0()

                else ->
                    if (it < 1) viewModel.setDisplayExerciseMinutesTo1()
            }

            if (it > 59) viewModel.setDisplayExerciseMinutesTo59()
        })

        viewModel.displayExerciseSeconds.observe(this, Observer {

            when {
                viewModel.displayExerciseMinutes.value!! >= 1 ->
                    if (it < 0) viewModel.setDisplayExerciseSecondsTo0()

                else ->
                    if (it < 1) viewModel.setDisplayExerciseSecondsTo1()
            }

            if (it > 59) viewModel.setDisplayExerciseSecondsTo59()
        })

        viewModel.displayRestMinutes.observe(this, Observer {

            when {
                viewModel.displayRestSeconds.value!! >= 1 ->
                    if (it < 0) viewModel.setDisplayRestMinutesTo0()

                else ->
                    if (it < 1) viewModel.setDisplayRestMinutesTo1()
            }

            if (it > 59) viewModel.setDisplayRestMinutesTo59()
        })

        viewModel.displayRestSeconds.observe(this, Observer {

            when {
                viewModel.displayRestMinutes.value!! >= 1 ->
                    if (it < 0) viewModel.setDisplayRestSecondsTo0()

                else ->
                    if (it < 1) viewModel.setDisplayRestSecondsTo1()
            }

            if (it > 59) viewModel.setDisplayRestSecondsTo59()
        })

        viewModel.patternRepeats.observe(this, Observer {

            if (it < 1) viewModel.setPatternRepeatsTo1()
        })
        ...
        viewModel.addNewPattern.observe(this, Observer {

            if (it) {
                binding.rvTimerPattern.scrollToPosition(viewModel.endOfTimerPatternList())
                viewModel.addNewPatternDOne()
            }
        })
